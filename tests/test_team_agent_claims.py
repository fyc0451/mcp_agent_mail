...
Acceptance (#988):
  * active member claims an unowned, non-routing, non-retired Agent with its
    registration_token -> owner set + binding created/revived atomically
  * constant-time token compare; token never leaks to responses
  * invalid token / non-member / foreign-owned / retired / routing agent rejected
  * re-claim is idempotent; concurrent double-claim yields one binding row
"""
...
async def _setup_team(client: AsyncClient, root: dict[str, str], slug: str = "core") -> None:
    """Global admin creates the group and joins it as an active member."""
...
async def _routing_agent(slug: str, name: str) -> int:
...
def _claim(client: AsyncClient, headers: dict[str, str], agent_id: int, token: str, slug: str = "core"):
    return client.post(
        f"/hub/api/projects/{slug}/agent-claims",
        headers=headers,
        json={"agent_id": agent_id, "registration_token": token},
    )
...
async def test_member_claims_unowned_agent(hub):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        claimed = await _claim(client, bob, agent_id, "tok-secret")
...
async def test_claim_rejects_invalid_token_without_leak(hub):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        wrong = await _claim(client, bob, agent_id, "wrong-token")
        assert wrong.status_code == 403
        assert "wrong-token" not in wrong.text
        assert "tok-secret" not in wrong.text

        # tokenless agent cannot be claimed at all
        tokenless_id = await _mk_agent("/workspaces/w2", "NoToken", token=None)
        no_token = await _claim(client, bob, tokenless_id, "whatever")
...
async def test_claim_requires_active_membership(hub):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        outsider = await _claim(client, carol, agent_id, "tok-secret")
...
async def test_claim_rejects_foreign_owned_and_retired_and_routing(hub):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        owned = await _claim(client, bob, foreign_id, "tok-secret")
        assert owned.status_code == 409

        # retired
        retired_id = await _mk_agent("/workspaces/w2", "OldDog", retired=True)
        retired = await _claim(client, bob, retired_id, "tok-secret")
        assert retired.status_code == 409

        # team routing agent
        routing_id = await _routing_agent("core", "TeamBot")
        routing = await _claim(client, bob, routing_id, "routing-tok")
...
async def test_reclaim_idempotent_and_unbind_revives(hub):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        first = await _claim(client, bob, agent_id, "tok-secret")
        assert first.status_code == 201
        binding_id = first.json()["binding"]["id"]

        again = await _claim(client, bob, agent_id, "tok-secret")
        ...
        revived = await _claim(client, bob, agent_id, "tok-secret")
...
async def test_concurrent_double_claim_is_atomic(hub):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        first, second = await asyncio.gather(
            _claim(client, bob, agent_id, "tok-secret"),
            _claim(client, bob, agent_id, "tok-secret"),
        )
...
async def test_claim_name_collides_with_active_handle(hub):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        agent_id = await _mk_agent("/workspaces/w1", "Bob")
        conflict = await _claim(client, bob, agent_id, "tok-secret")
